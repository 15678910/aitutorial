...
print(상자.사용("글자수", "안녕하세요"))`,
  ...
  // ═══════════════════════════════════════════
  // 미니 프로젝트: AI 포트폴리오 실전 프로젝트
  // ═══════════════════════════════════════════

  'ai-portfolio-ch6-s1': {
    title: '🤖 나만의 AI 챗봇 만들기',
    description: '규칙 기반으로 동작하는 나만의 한국어 AI 챗봇을 만들어보세요! 키워드를 인식하고 적절한 답변을 해주는 똑똑한 챗봇입니다.',
    initialCode: `# 나만의 AI 챗봇 만들기
import random
...
class 나의챗봇:
    def __init__(self, 이름):
        ...
        self.대화기록 = []
        self.응답사전 = {
            "안녕": ["안녕하세요! 반가워요 😊", "하이! 오늘 기분이 어때요?", "안녕! 무엇을 도와줄까요?"],
            "이름": [f"제 이름은 {이름}이에요!", f"저는 {이름}이라고 해요~ 만나서 반가워요!"],
            "날씨": ["오늘 날씨가 좋다면 산책 어때요? 🌤️", "비가 와도 괜찮아요, 실내에서 코딩하면 되죠! 💻"],
            "공부": ["공부는 매일 조금씩 하는 게 최고예요! 📚", "어려운 건 천천히, 쉬운 건 빠르게! 화이팅! 💪"],
            "게임": ["게임도 좋지만 코딩이 더 재밌어요! 😄", "프로그래밍으로 게임을 만들어보는 건 어때요?"],
        }

    def 키워드찾기(self, 입력문장):
        for 키워드 in self.응답사전:
            if 키워드 in 입력문장:
                return 키워드
        return None

    def 대답하기(self, 입력문장):
        self.대화기록.append(f"사용자: {입력문장}")
        키워드 = self.키워드찾기(입력문장)
        if 키워드:
            응답 = random.choice(self.응답사전[키워드])
        else:
            응답 = "음... 잘 모르겠어요. 다른 것을 물어봐주세요! 🤔"
        self.대화기록.append(f"{self.이름}: {응답}")
        return 응답

    def 대화요약(self):
        print(f"\\n📊 대화 요약 (총 {len(self.대화기록)}줄)")
        print(f"알고 있는 주제: {', '.join(self.응답사전.keys())}")

# 챗봇 만들기
봇 = 나의챗봇("다솜이")

# 대화 테스트
대화목록 = ["안녕하세요!", "오늘 날씨 어때?", "공부하기 싫어", "피자 먹고 싶어"]

print(f"🤖 {봇.이름} 챗봇과 대화하기!\\n")
for 말 in 대화목록:
    print(f"👤 나: {말}")
    답변 = 봇.대답하기(말)
    print(f"🤖 {봇.이름}: {답변}\\n")
...
봇.대화요약()`,
    expectedOutput: '🤖 다솜이 챗봇과 대화하기!',
    hint: '키워드를 기반으로 응답을 선택하는 규칙 기반 챗봇이에요. 응답사전에 새로운 키워드와 응답을 추가해서 더 똑똑하게 만들어보세요!',
    packages: [],
  },

  'ai-portfolio-ch6-s2': {
    title: '🎬 AI 추천 시스템 만들기',
    description: '사용자의 취향을 분석해서 영화나 책을 추천해주는 AI 추천 시스템을 만들어보세요!',
    initialCode: `# AI 추천 시스템 만들기

class 추천시스템:
    def __init__(self):
        self.콘텐츠목록 = {
            "해리포터": {"장르": "판타지", "분위기": "모험", "평점": 4.8},
            "어벤져스": {"장르": "액션", "분위기": "모험", "평점": 4.5},
            "겨울왕국": {"장르": "애니메이션", "분위기": "감동", "평점": 4.7},
            "코코": {"장르": "애니메이션", "분위기": "감동", "평점": 4.9},
            "주토피아": {"장르": "애니메이션", "분위기": "모험", "평점": 4.6},
            "스파이더맨": {"장르": "액션", "분위기": "모험", "평점": 4.4},
            "토이스토리": {"장르": "애니메이션", "분위기": "감동", "평점": 4.8},
            "인사이드아웃": {"장르": "애니메이션", "분위기": "감동", "평점": 4.7},
        }

    def 취향분석(self, 좋아하는것들):
        장르점수 = {}
        분위기점수 = {}
        for 제목 in 좋아하는것들:
            if 제목 in self.콘텐츠목록:
                정보 = self.콘텐츠목록[제목]
                장르점수[정보["장르"]] = 장르점수.get(정보["장르"], 0) + 1
                분위기점수[정보["분위기"]] = 분위기점수.get(정보["분위기"], 0) + 1
        선호장르 = max(장르점수, key=장르점수.get) if 장르점수 else None
        선호분위기 = max(분위기점수, key=분위기점수.get) if 분위기점수 else None
        return 선호장르, 선호분위기

    def 추천하기(self, 좋아하는것들, 개수=3):
        선호장르, 선호분위기 = self.취향분석(좋아하는것들)
        print(f"📊 취향 분석 결과")
        print(f"  선호 장르: {선호장르}")
        print(f"  선호 분위기: {선호분위기}\\n")

        점수목록 = []
        for 제목, 정보 in self.콘텐츠목록.items():
            if 제목 not in 좋아하는것들:
                매칭점수 = 0
                if 정보["장르"] == 선호장르:
                    매칭점수 += 2
                if 정보["분위기"] == 선호분위기:
                    매칭점수 += 1
                매칭점수 += 정보["평점"] / 5
                점수목록.append((제목, 매칭점수, 정보))
        점수목록.sort(key=lambda x: x[1], reverse=True)

        print(f"🎬 추천 결과 (상위 {개수}개):")
        for 순위, (제목, 점수, 정보) in enumerate(점수목록[:개수], 1):
            별 = "⭐" * int(정보["평점"])
            print(f"  {순위}. {제목} ({정보['장르']}/{정보['분위기']}) {별}")

# 추천 시스템 사용하기
추천기 = 추천시스템()
내가본것 = ["겨울왕국", "코코"]

print("🎯 나의 시청 기록:", ", ".join(내가본것))
print("=" * 40 + "\\n")
추천기.추천하기(내가본것)`,
    expectedOutput: '🎯 나의 시청 기록: 겨울왕국, 코코\n========================================\n\n📊 취향 분석 결과\n  선호 장르: 애니메이션\n  선호 분위기: 감동\n\n🎬 추천 결과 (상위 3개):\n  1. 토이스토리 (애니메이션/감동) ⭐⭐⭐⭐\n  2. 인사이드아웃 (애니메이션/감동) ⭐⭐⭐⭐\n  3. 주토피아 (애니메이션/모험) ⭐⭐⭐⭐',
    hint: '추천 시스템은 사용자의 과거 기록에서 패턴을 찾아 비슷한 콘텐츠를 추천해요. 장르와 분위기 매칭 점수를 계산하는 방식이에요!',
    packages: [],
  },

  'ai-portfolio-ch6-s3': {
    title: '🧠 AI 퀴즈 게임 만들기',
    description: '자동으로 채점하고 결과를 분석해주는 AI 퀴즈 게임을 만들어보세요!',
    initialCode: `# AI 퀴즈 게임 만들기
...
class AI퀴즈게임:
    ...
    def 문제추가(self, 질문, 보기들, 정답번호, 난이도="보통"):
        self.문제들.append({
            "질문": 질문,
            "보기": 보기들,
            "정답": 정답번호,
    ...
    def 시험보기(self, 답안들):
        self.결과 = []
        for i, (문제, 제출답) in enumerate(zip(self.문제들, 답안들)):
            맞음 = 제출답 == 문제["정답"]
            self.결과.append({
                "번호": i + 1,
                "맞음": 맞음,
    ...
    def 결과분석(self):
        총문제 = len(self.결과)
        맞은수 = sum(1 for r in self.결과 if r["맞음"])
        점수 = round(맞은수 / 총문제 * 100)

        print(f"📝 {self.제목} - 시험 결과\\n")
        print(f"{'='*35}")

        for r in self.결과:
            표시 = "✅" if r["맞음"] else "❌"
            문제 = self.문제들[r["번호"]-1]
            print(f"{표시} {r['번호']}번: {문제['질문'][:20]}...")
            if not r["맞음"]:
                print(f"   → 정답: {r['정답']}번, 제출: {r['제출']}번")

        print(f"\\n{'='*35}")
        print(f"점수: {맞은수}/{총문제} ({점수}점)")
        ...
        print(f"\\n📊 난이도별 정답률:")
        for 난이도, 데이터 in 난이도별.items():
            비율 = round(데이터["맞음"] / 데이터["총"] * 100)
            막대 = "█" * (비율 // 10)
            print(f"  {난이도}: {막대} {비율}%")
...
퀴즈.문제추가("AI는 무엇의 약자인가요?", ["Apple Intelligence", "Artificial Intelligence", "Auto Input"], 2, "쉬움")
퀴즈.문제추가("머신러닝에서 데이터로 학습하는 과정은?", ["훈련", "테스트", "삭제"], 1, "보통")
퀴즈.문제추가("딥러닝의 기본 단위는?", ["셀", "뉴런", "바이트"], 2, "보통")
퀴즈.문제추가("AI가 고양이 사진을 구별하는 것은?", ["분류", "생성", "삭제"], 1, "쉬움")
퀴즈.문제추가("GPT에서 T는 무엇인가요?", ["Training", "Transformer", "Testing"], 2, "어려움")
...
퀴즈.시험보기(학생답안)
퀴즈.결과분석()`,
    expectedOutput: '📝 AI 기초 퀴즈 - 시험 결과\n\n===================================\n✅ 1번: AI는 무엇의 약자인가요?...\n✅ 2번: 머신러닝에서 데이터로 학습하는 과정은...\n✅ 3번: 딥러닝의 기본 단위는?...\n✅ 4번: AI가 고양이 사진을 구별하는 것...\n❌ 5번: GPT에서 T는 무엇인가요?...\n   → 정답: 2번, 제출: 1번\n\n===================================\n점수: 4/5 (80점)\n등급: 🥈 우수\n\n📊 난이도별 정답률:\n  쉬움: ██████████ 100%\n  보통: ██████████ 100%\n  어려움: 0%',
    hint: '퀴즈 게임은 문제를 저장하고, 답안을 채점하고, 결과를 분석하는 세 단계로 이루어져요. 난이도별 분석까지 하면 더 유용해요!',
    packages: [],
  },

  'ai-portfolio-ch6-s4': {
    title: '📊 데이터 시각화 프로젝트',
    description: '데이터를 텍스트 기반 막대 차트로 시각화하는 프로그램을 만들어보세요! 실제 데이터 분석의 기초를 배울 수 있어요.',
    initialCode: `# 데이터 시각화 프로젝트
...
class 데이터차트:
    ...
    def 데이터추가(self, 카테고리, 항목들):
        self.데이터셋[카테고리] = 항목들

    def 막대차트(self, 카테고리):
        if 카테고리 not in self.데이터셋:
            print("데이터가 없어요!")
            return
        ...
        for 이름, 값 in 항목들.items():
            막대길이 = int(값 / 최대값 * 25)
            막대 = "█" * 막대길이
            빈칸 = "░" * (25 - 막대길이)
            print(f"  {이름:>6} │{막대}{빈칸}│ {값}")

        print(f"{'─' * 40}")

    def 비교차트(self, 카테고리1, 카테고리2):
        데이터1 = self.데이터셋.get(카테고리1, {})
        데이터2 = self.데이터셋.get(카테고리2, {})
        모든키 = list(데이터1.keys())

        print(f"\\n📈 비교: {카테고리1} vs {카테고리2}")
        print(f"{'─' * 40}")
        print(f"  {'항목':>6} │ {카테고리1:>6} → {카테고리2:>6} │ 변화")
        print(f"{'─' * 40}")
        ...
        for 키 in 모든키:
            ...
            값2 = 데이터2.get(키, 0)
            변화 = 값2 - 값1
            화살표 = "📈" if 변화 > 0 else "📉" if 변화 < 0 else "➡️"
            print(f"  {키:>6} │ {값1:>6} → {값2:>6} │ {화살표} {변화:+}")
    ...
    def 통계요약(self, 카테고리):
        ...
        값들 = list(self.데이터셋[카테고리].values())
        정렬 = sorted(값들)

        print(f"\\n📋 통계 요약 ({카테고리})")
        print(f"  합계: {sum(값들)}")
        print(f"  평균: {sum(값들)/len(값들):.1f}")
        print(f"  최솟값: {min(값들)}")
        print(f"  최댓값: {max(값들)}")
...
차트.데이터추가("중간고사", {"수학": 85, "영어": 78, "과학": 92, "국어": 88, "사회": 75})
차트.데이터추가("기말고사", {"수학": 90, "영어": 85, "과학": 88, "국어": 92, "사회": 82})

차트.막대차트("중간고사")
차트.비교차트("중간고사", "기말고사")
차트.통계요약("중간고사")`,
    expectedOutput: '\n📊 우리 반 성적 - 중간고사\n────────────────────────────────────────\n    수학 │█████████████████████████░░░░│ 85\n    영어 │█████████████████████░░░░░░░│ 78\n    과학 │█████████████████████████████│ 92\n    국어 │███████████████████████████░░│ 88\n    사회 │████████████████████░░░░░░░░│ 75\n────────────────────────────────────────',
    hint: '텍스트로도 멋진 차트를 만들 수 있어요! 최대값 대비 비율로 막대 길이를 계산하는 것이 핵심이에요.',
    packages: [],
  },

  'ai-portfolio-ch6-s5': {
    title: '📔 AI 일기 분석기',
    description: '일기를 분석해서 감정을 판별하고, 키워드를 추출하는 AI 일기 분석기를 만들어보세요!',
    initialCode: `# AI 일기 분석기

class 일기분석기:
    def __init__(self):
        self.긍정단어 = ["좋았다", "행복", "즐거웠다", "기뻤다", "재밌었다", "신났다",
                       "웃었다", "맛있었다", "예뻤다", "감동", "최고", "성공", "잘했다"]
        self.부정단어 = ["슬펐다", "힘들었다", "짜증", "화났다", "아팠다", "실패",
                       "무서웠다", "외로웠다", "지루했다", "싫었다", "울었다", "속상했다"]
        self.활동단어 = {
            "🏫 학교": ["학교", "수업", "선생님", "숙제", "시험", "공부"],
            "👫 친구": ["친구", "같이", "놀았다", "만났다"],
            "🏠 집": ["집", "엄마", "아빠", "가족", "동생"],
            "⚽ 운동": ["운동", "축구", "달리기", "수영", "체육"],
            "🎮 취미": ["게임", "그림", "음악", "책", "영화", "코딩"],
    ...
    def 감정분석(self, 텍스트):
        긍정수 = sum(1 for 단어 in self.긍정단어 if 단어 in 텍스트)
        부정수 = sum(1 for 단어 in self.부정단어 if 단어 in 텍스트)
        총점 = 긍정수 - 부정수
    ...
    def 활동추출(self, 텍스트):
        발견활동 = []
        for 카테고리, 키워드들 in self.활동단어.items():
            if any(키 in 텍스트 for 키 in 키워드들):
                발견활동.append(카테고리)
        return 발견활동
    ...
    def 분석리포트(self):
        ...
        for 일기 in self.일기목록:
            ...
            if 점수 > 0:
                ...
                막대 = "⬜"
            print(f"  {일기['날짜']}: {막대} ({점수:+d})")

        # 통계
        평균점수 = sum(d["점수"] for d in self.일기목록) / len(self.일기목록)
        print(f"\\n📊 평균 감정 점수: {평균점수:+.1f}")

# 일기 분석기 사용하기
분석기 = 일기분석기()

분석기.일기쓰기("월요일", "학교에서 수업을 듣고 친구와 같이 놀았다. 재밌었다!")
분석기.일기쓰기("화요일", "시험이 어려웠다. 힘들었다. 집에서 공부했다.")
분석기.일기쓰기("수요일", "축구를 했다. 골을 넣어서 기뻤다! 최고의 하루!")
분석기.일기쓰기("목요일", "비가 와서 지루했다. 집에서 게임을 했다.")
분석기.일기쓰기("금요일", "친구 생일파티에서 맛있었다. 행복한 하루! 웃었다.")

분석기.분석리포트()`,
    expectedOutput: '📔 AI 일기 분석 리포트\n========================================',
    hint: '감정 분석은 긍정 단어와 부정 단어의 개수를 세어서 점수를 계산해요. 키워드 매칭은 실제 AI의 자연어 처리(NLP)의 기초가 되는 방법이에요!',
    packages: [],
  },
}